...
int FRC_CAN_isMe(unsigned long int canId, unsigned long int myId) {  // like HW match but also requires matching device number.
  ...
  unsigned long myMask = INCAN_MASK | FRC_DEVNUM_MASK;
...
int FRC_CAN_isRIO(unsigned long canId) { // makes strong assumptions about the ID’s of the control computer
  int ret = 0;
  unsigned long tmp;
  unsigned long riomask = RIO_MASK;;
  tmp = canId & RIO_MASK;
  if (tmp == riomask) {
    ret = 1;
  } else {
